...
const InboxQuickButton = ({ onClick, isOpen }) => {
  return (
    <button
      onClick={onClick}
      className={`relative z-10 cursor-pointer hover:opacity-90 active:opacity-80 rounded-full h-[60px] w-[60px] flex items-center justify-center ${
        isOpen ? "bg-indicator-violet" : "bg-primary-gray-light"
      }`}
    >
      <svg
        className="h-7 w-7"
...
InboxQuickButton.propTypes = {
  onClick: PropTypes.func,
  isOpen: PropTypes.bool,
};

export default InboxQuickButton;
